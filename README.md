...
2. **Native Libraries & TDLib Wrapper**:
   ...
   - **CRITICAL**: For a production build, you MUST replace the `TdApi.java` file in this project with the full, autogenerated `TdApi.java` from the official TDLib source to ensure 100% JNI compatibility. The stubs provided here contain only the fields used by this app and may cause `NoSuchFieldError` crashes if the native library expects missing fields.

...
interface TransferDao {
    ...
    @Query("SELECT * FROM transfers WHERE status = 'IN_PROGRESS' OR status = 'PENDING'")
    ...
    @Query("UPDATE transfers SET status = :status, progress = :progress, transferredSize = :transferredSize WHERE id = :id")
    suspend fun updateProgress(id: Long, status: TransferStatus, progress: Float, transferredSize: Long)

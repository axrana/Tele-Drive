...
import com.example.teledrive.transfers.UploadWorker
...
class FileExplorerViewModel(
    ...
    private val _currentFolderId = MutableStateFlow<Long?>(savedStateHandle.get<Long>("folder_id"))
    ...
    private val _selectedFolders = MutableStateFlow<Set<Long>>(emptySet())
    val selectedFolders: StateFlow<Set<Long>> = _selectedFolders.asStateFlow()

    val isSelectionMode: StateFlow<Boolean> = combine(_selectedFiles, _selectedFolders) { files, folders ->
                ...
                if (tdFile.local.isDownloadingCompleted) {
                    ...
                    if (entity != null) {
                        ...
                        if (_pendingDownloads.value.contains(tdFile.remote.id)) {
                            ...
                            val success = com.example.teledrive.storage.UriUtils.saveToDownloads(
                                context,
        ...
        viewModelScope.launch {
            ...
            if (file.local.isDownloadingCompleted) {
                val uri = androidx.core.content.FileProvider.getUriForFile(
                    ...
                    java.io.File(file.local.path)
                )
                val intent = android.content.Intent(android.content.Intent.ACTION_VIEW).apply {
                    setDataAndType(uri, fileEntity.mimeType ?: com.example.teledrive.storage.UriUtils.getMimeType(fileEntity.name))
                    addFlags(android.content.Intent.FLAG_GRANT_READ_URI_PERMISSION)
                }
                context.startActivity(android.content.Intent.createChooser(intent, "Open with"))
            } else {
                downloadFile(fileEntity)
                _errorFlow.emit("File not downloaded yet. Starting download...")
            }
        }
    }
    ...
    fun navigateToFolder(folder: Folder?) {
        ...
        savedStateHandle.set("folder_id", folder?.id)
        if (folder == null) {
    ...
    fun uploadFile(context: Context, uri: android.net.Uri, shouldCompress: Boolean) {
        viewModelScope.launch {
            try {
                ...
                val realFile = com.example.teledrive.storage.UriUtils.getFileFromUri(context, uri)
                    ?: run {
                ...
                transferManager.enqueueUpload(
                    localPath = realFile.absolutePath,
                    folderId = folderId,
                    name = realFile.name,
                    size = realFile.length(),
                    mimeType = com.example.teledrive.storage.UriUtils.getMimeType(realFile.name)
                )
                _errorFlow.emit("Upload enqueued: ${realFile.name}")
            } catch (e: Exception) {
    ...
    fun downloadFile(fileEntity: FileEntity) {
        viewModelScope.launch {
            try {
                ...
                _errorFlow.emit("Download enqueued: ${fileEntity.name}")
            } catch (e: Exception) {
    ...
    fun toggleFileSelection(fileId: Long) {
        _selectedFiles.value = if (_selectedFiles.value.contains(fileId)) {
    ...
    fun selectAll() {
        _selectedFiles.value = files.value.map { it.id }.toSet()
    ...
    fun deleteSelected() {
        viewModelScope.launch {
            try {
                ...
                // Delete Files
                _selectedFiles.value.forEach { id ->
                ...
                // Delete Folders
                _selectedFolders.value.forEach { id ->
                ...
                clearSelection()
                _errorFlow.emit("Bulk delete completed")
            } catch (e: Exception) {
                _errorFlow.emit("Bulk delete failed: ${e.message}")
            }
        }
    }
    ...
    fun downloadSelected() {
        viewModelScope.launch {
            _selectedFiles.value.forEach { id ->
                repository.getFileById(id)?.let { downloadFile(it) }
            }
            clearSelection()
            _errorFlow.emit("Bulk download started")
        }
    }

    val allTransfers: Flow<List<com.example.teledrive.data.local.entity.TransferEntity>> = transferManager.allTransfers

    fun cancelTransfer(transfer: com.example.teledrive.data.local.entity.TransferEntity) {
        viewModelScope.launch {
